...
// The two ways `export_pdf` prints on Windows, in a module of their own.
#[cfg(target_os = "windows")]
mod webview2;
...
pub async fn export_pdf(
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        match webview2::print_through_devtools(&window, params).await {
            Ok(pdf) => std::fs::write(&path, pdf).map_err(|e| e.to_string())?,
            Err(reason) => {
                eprintln!(
                    "the DevTools protocol did not print ({reason}); printing without bookmarks"
                );
                webview2::print_to_pdf(&window, &path, custom_page, paged, paper.as_deref(), loc)
                    .await?;
            }
        }
