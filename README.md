...
- **Title, author and date** from the front-matter become a title block at the head of the document. With a title there, the running head carries the document's name on every page instead of the chapter it is in, and the exported PDF is titled with it rather than with the file's name — a Marp deck's too. On the desktop, the PDF's properties also carry the `author`, `subject` and `keywords` the front-matter gives, in Pandoc's keys and with its lists (`keywords: [pdf, maps]`, or one `- item` a line).
...
- **The language, from the document**: `lang: es` in the front-matter, Pandoc's key again, says what the document is written in. The preview, the editor and the exported HTML carry it, so hyphenation, spell checking and screen readers can go by it, and a right-to-left language turns the page with it. Quotation marks follow it too: `"…"` comes out «…» in Spanish, as the RAE recommends for print, „…“ in German, and « … » with narrow spaces in French, from CLDR's data for the rest. Without `lang:` everything follows the interface as before, and quotation marks stay “…” whoever opens the file.
...
- **Cross-references**, in pandoc-crossref's syntax: label a figure `![…](…){#fig:dot}`, a table's caption `Table: … {#tbl:times}` or an equation `$$ … $$ {#eq:energy}`, and refer to it with `@fig:dot` ("fig. 1", in the interface language), `@Fig:dot` to start a sentence, or `-@fig:dot` for the number alone. Each reference is a link to what it names, in both views and in the HTML export; one to a label the document does not have shows as **¿fig:dot?**. A labelled image is a figure even without a title, captioned by its alt text, and labelled equations are numbered among themselves.
